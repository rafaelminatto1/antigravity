-- Script para aplicar migration diretamente via SQL Editor do Supabase
...
-- Criar índices para patients se não existirem
...
-- Criar índices para appointments se não existirem
...
BEGIN
  -- Organizations
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'organizations' AND policyname = 'Users can view organizations they belong to') THEN
  ...
  -- Sessions
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sessions' AND policyname = 'Users can view sessions in their org') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'sessions' AND policyname = 'Physiotherapists can manage sessions') THEN
  ...
  -- Body Pain Maps
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'body_pain_maps' AND policyname = 'Users can view pain maps in their org') THEN
  ...
  -- Waitlist
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'waitlist' AND policyname = 'Users can view waitlist in their org') THEN
